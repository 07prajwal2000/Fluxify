...
import { validatePlaygroundRequest, type PlaygroundValidationErrors } from "./validation";

const toObject = (rows: ApiKeyValue[]) => Object.fromEntries(rows.filter((row) => row.key).map((row) => [row.key, row.value]));

export function ApiPlayground({ route, baseUrl = "", onSend, className, isFramed = true, initialPathParams, initialQuery, initialHeaders, initialBody, initialState, defaultValidate = true, onRequestChange, onStateChange }: ApiPlaygroundProps) {
	...
	const [validateBeforeSend, setValidateBeforeSend] = useState(initialState?.validateBeforeSend ?? defaultValidate ?? true);
	...
	useEffect(() => {
		onStateChange?.({ pathRows, queryRows, headerRows, contentType, body, formBody, response, validateBeforeSend });
	}, [onStateChange, pathRows, queryRows, headerRows, contentType, body, formBody, response, validateBeforeSend]);
	...
	return <Card style={{ padding: 0, gap: 0 }} className={clsx("flex h-full min-h-0 flex-col overflow-hidden bg-background text-foreground", isFramed ? "border-border" : "rounded-none border-0", className)}>
		<div className="shrink-0 border-b border-border px-4 py-3">
			<div className="flex h-10 gap-2">
				<span style={{ width: 78 }} className="inline-flex shrink-0 items-center justify-center rounded-l-md border border-border bg-surface font-mono text-[13px] font-bold text-accent">{route.method}</span>
				<Input aria-label="Request URL" readOnly value={rawPath} className="h-10 min-w-0 flex-1 rounded-r-md font-mono text-[13px]" />
				<Button isDisabled={isSending} onPress={send} style={{ width: 112 }} className="h-10 shrink-0 font-semibold"><span className="grid w-4 place-items-center">{isSending ? <TbLoader2 className="animate-spin" size={16} /> : <TbPlayerPlayFilled size={15} />}</span><span>{isSending ? "Sending" : "Send"}</span></Button>
			</div>
