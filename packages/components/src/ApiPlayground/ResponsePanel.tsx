...
export function ResponsePanel({ response }: { response?: ApiPlaygroundResponse }) {
	...
	const responseBody = useMemo(
		() => formatResponseBody(response?.body ?? "", mimeType),
	...
	return (
		<section className="flex min-h-0 flex-col overflow-hidden bg-background">
			<Tabs
				...
				className="api-playground-tabs api-playground-tabs--response flex min-h-0 flex-1 flex-col"
			>
				<Tabs.ListContainer className="shrink-0 px-3">
					<div className="api-playground-response-tabs__bar">
						...
						<Tabs.List
							aria-label="Response contents"
							style={{ width: "max-content", minWidth: 0, flex: "0 0 auto" }}
							className="gap-5"
						>
							<Tabs.Tab id="body">
								Body
					...
					{response ? (
						<>
							...
							<Button
								...
								onPress={() => navigator.clipboard.writeText(responseBody)}
							>
								<TbCopy size={14} />
							</Button>
						</>
				...
				<Tabs.Panel id="headers" className="min-h-0 flex-1 overflow-hidden p-3">
					{headers.length ? (
						<div className="overflow-hidden rounded-md border border-border">
							<div className="grid grid-cols-[minmax(140px,30%)_1fr] border-b border-border bg-surface px-3 py-2 font-mono text-[10px] uppercase tracking-wider text-muted">
								<span>Name</span>
								<span>Value</span>
							</div>
							{headers.map(([key, value]) => (
								<div
									className="grid grid-cols-[minmax(140px,30%)_1fr] border-b border-border px-3 py-2 font-mono text-xs last:border-0"
									key={key}
								>
...
function ResponseMetadata({
	...
	return (
		<div className="flex shrink-0 items-center gap-3">
			<span
				className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-1 font-mono text-[11px] font-semibold ${statusTone(response.status)}`}
			>
...
function EmptyResponse() {
	return (
		<div className="grid h-full place-items-center p-6 text-center">
			<div>
				<div className="mb-2 font-mono text-xs font-semibold">NO RESULT</div>
				<p className="max-w-60 text-xs text-muted">
					Send this request to inspect its response, headers, timing, and size.
				</p>
			</div>
		</div>
	);
