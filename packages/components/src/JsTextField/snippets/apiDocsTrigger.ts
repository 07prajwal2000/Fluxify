...
export const TRIGGER_DOCS: ApiDocItem[] = [
	{
		...
		description: "Array of incoming trigger events with payload and metadata. Always an array, even for single events.",
		category: "variables",
		example: "// Incoming batch events array\nconst events = trigger.data;\nconst firstEvent = events[0]?.data;",
	...
	},
	{
		...
		description: 'Execution origin type indicating what initiated the run ("route", "job", "workflow", "cron", or "trigger").',
		category: "variables",
	...
	},
	{
		...
		description: 'Reply mode: "sync" when caller waits for response, or "async" for fire-and-forget.',
		category: "variables",
	...
	},
	{
		...
		category: "variables",
		example: "const { batchId, size, attempt } = trigger.meta;\nlogger.logInfo({ batchId, size, attempt });",
	...
	},
	{
		...
		kind: "object",
		signature: "const trigger.connection?: { commit(): Promise<void>; moveToDLQ(error?: any): Promise<void>; lag(): Promise<number | null>; };",
		description: "Queue connection controls for external message stream triggers (Kafka, NATS): manual commit, moveToDLQ, and lag.",
		category: "variables",
