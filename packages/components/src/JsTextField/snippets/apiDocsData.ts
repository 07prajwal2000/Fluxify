...
export const API_DOCS: ApiDocItem[] = [
	...
	// JWT
	{
		...
		category: "auth",
		example: 'const token = jwt.sign({ sub: input.id }, getConfig("JWT_SECRET") || "secret", { expiresIn: "1d" });',
		returns: "string",
	},
	{
		id: "api-jwt-verify",
		name: "jwt.verify",
		kind: "function",
		signature: "jwt.verify(token: string, secretKey: string, options?: object): { success: boolean; payload: any }",
	...
	// HTTP client
	{
		...
		name: "httpClient.get",
		kind: "function",
		signature: "httpClient.get<T = any>(url: string, headers?: Record<string, string>): Promise<AxiosResponse<T>>",
	...
	},
	{
		...
		kind: "function",
		signature: "httpClient.post<T = any>(url: string, data?: any, headers?: Record<string, string>): Promise<AxiosResponse<T>>",
		description: "Sends an asynchronous HTTP POST request with JSON payload.",
		category: "http",
		example: 'const res = await httpClient.post("https://api.example.com/items", { name: "New Item" });\nreturn res.data;',
		returns: "Promise<AxiosResponse<T>>",
	},
	{
		id: "api-http-put",
		name: "httpClient.put",
		kind: "function",
		signature: "httpClient.put<T = any>(url: string, data?: any, headers?: Record<string, string>): Promise<AxiosResponse<T>>",
		description: "Sends an asynchronous HTTP PUT request.",
		category: "http",
		example: 'const res = await httpClient.put("https://api.example.com/items/1", { name: "Updated" });',
		returns: "Promise<AxiosResponse<T>>",
	},
	{
		id: "api-http-delete",
		...
		kind: "function",
		signature: "httpClient.delete<T = any>(url: string, headers?: Record<string, string>): Promise<AxiosResponse<T>>",
	...
	},
	{
		...
		kind: "function",
		signature: "httpClient.patch<T = any>(url: string, data?: any, headers?: Record<string, string>): Promise<AxiosResponse<T>>",
		description: "Sends an asynchronous HTTP PATCH request.",
		category: "http",
		example: 'const res = await httpClient.patch("https://api.example.com/items/1", { active: true });',
		returns: "Promise<AxiosResponse<T>>",
	},
	...
	// Database
	{
		...
		description: "Executes parameterized SQL query with $1, $2 placeholders (PostgreSQL) or ? (MySQL).",
		category: "database",
		example: 'const rows = await dbQuery("SELECT * FROM users WHERE id = $1", [input.id]);\nreturn rows;',
	...
	},
	{
		...
		category: "validation",
		example: 'const schema = libs.zod.object({ email: libs.zod.string().email() });\nconst result = schema.safeParse(input);',
		returns: "Zod",
	},
];
