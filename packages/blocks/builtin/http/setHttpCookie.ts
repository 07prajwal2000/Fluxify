...
export const setHttpCookieBlockSchema = z
	.object({
		...
		value: z.string().or(z.number()).describe("cookie value (supports js expression)"),
		domain: z.string().optional().describe("domain (supports js expression)"),
		path: z.string().optional().describe("http path for the cookie (supports js expression)"),
		expiry: z.string().describe("expiry in date (ISO format, supports js expression)"),
		httpOnly: z.boolean().optional().describe("httponly cookie?"),
		secure: z.boolean().optional().describe("only in https?"),
		samesite: z.enum(HttpCookieSameSite).optional().describe("cookie samesite setting"),
	})
