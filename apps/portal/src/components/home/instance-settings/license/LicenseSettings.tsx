...
function statusChip(view: LicenseView): { label: string; color: ChipColor } {
	if (view.status === "invalid")
		return { label: "Invalid key", color: "danger" };
...
export function LicenseSettings() {
	const {
		...
		isLoading,
		isError,
	} = instanceSettingsQuery.license.useQuery();
	...
	const running =
		view.status !== "invalid" &&
		(view.status !== "expired" || !!view.daysRemaining);
	const unlocks = (feature?: string) =>
		!feature ||
		(running &&
			(view.features.includes("*") || view.features.includes(feature)));
		...
		<div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
			<div>
				...
				<p className="mt-0.5 text-sm text-muted-foreground">
					Which edition this instance runs and what it unlocks. Changes reach
					every worker within seconds — no restart needed.
				</p>
			</div>
			...
			<section className="flex flex-col gap-4 rounded-xl border border-border bg-background p-5">
				...
				{view.invalidReason && (
					<p className="text-xs text-danger">
						The key was rejected ({view.invalidReason}), so this instance runs
						as Community.
					</p>
				)}
				<Details view={view} />
				{view.source === "env" && (
					<p className="flex items-start gap-2 rounded-lg bg-surface-secondary p-3 text-xs text-muted">
						...
						Managed by the environment. The LICENSE_KEY variable on the admin
						container sets the license, so it can't be changed here. Remove it
						and restart to manage the license from this page.
					</p>
				)}
			</section>
...
function FeatureState({
	...
	on: boolean;
	off: boolean | string | undefined;
}) {
	if (!on)
		return (
			<TbMinus size={16} className="text-muted" aria-label="Not included" />
		);
	if (off)
		return (
			<span className="text-xs text-warning">
				Included · switched off on this instance
			</span>
		);
...
function Details({ view }: { view: LicenseView }) {
	...
	if (view.graceEndsAt)
		rows.push(["Grace period ends", date(view.graceEndsAt)]);
	if (view.fingerprint)
		rows.push([
			"Key fingerprint",
