...
import { useArtifactParams, useBlockingParent, useCanvasArtifact } from "./useArtifact";
...
export function CanvasArtifact({ subArtifactId }: { subArtifactId: string }) {
	...
	const { graph, route, routeId, routeExists, targetsRoute } = useCanvasArtifact(detail);
	...
	return (
		<div className="flex flex-col gap-4">
			<div>
				<span className="text-[10px] text-muted uppercase font-bold tracking-wider">Target</span>
			...
			{routeExists || blockingParent || !targetsRoute ? (
				...
				<div className="rounded-lg border border-warning/30 bg-warning/10 p-3">
					<p className="text-xs text-foreground">
						These changes hang off a route that doesn't exist yet. Create the route first, then come
						back and apply them.
					</p>
				</div>
			)}
