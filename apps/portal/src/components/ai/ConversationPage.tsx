...
export function ConversationPage() {
	...
	const {
		...
		hasNextPage,
		isFetchingNextPage,
	} = harnessConversationsQuery.messages.useInfiniteQuery(projectId, conversationId);
	...
	const planText =
		activeRun?.hitl?.markdownPlan ||
		(isPlanReviewMode && messages.length > 0 ? messages[0].aiResponse : null);
					...
					<div className="mx-auto flex w-full max-w-[65%] flex-col-reverse gap-6">
						...
						{isLoading ? (
										...
										<div className="flex items-start gap-4">
											...
											<div className="flex-1">
												...
												<p className="text-sm text-muted mt-1 leading-relaxed">
													The AI has proposed an implementation plan. Please review the blocks,
													provide feedback, or approve it to proceed.
												</p>
											</div>
										</div>
										<Button
							...
							messages.map((msg, idx) => {
								...
								const isHitlMessage =
									msg.status === "awaiting_hitl" || msg.status.startsWith("hitl_");
										...
										{isActionablePlan && planText ? (
											...
											<div className="flex w-full flex-col gap-2">
												...
												<AiMessage
													...
													status={isRunActive ? undefined : displayStatus}
													usage={msg.usage}
												/>
											</div>
										)}
