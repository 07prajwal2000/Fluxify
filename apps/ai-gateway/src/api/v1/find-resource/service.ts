 ...
 * caps itself at 10 rows, so the response stays bounded at 40.
 *
...
export default async function handleRequest(projectId: string, q: string) {
	const [routes, integrations, appConfigs, customBlocks] = await Promise.all([
		dbService.findRoutes(projectId, q),
		dbService.findIntegrations(projectId, q),
		dbService.findAppConfigs(projectId, q),
		dbService.findCustomBlocks(projectId, q),
	]);

	return {
		query: q,
		results: [...routes, ...integrations, ...appConfigs, ...customBlocks],
	};
}
