...
const USER_QUERY_TRUNCATE_LEN = 30;

function truncateUserQuery(query?: string | null): string | undefined {
	if (!query) return undefined;
	return query.length > USER_QUERY_TRUNCATE_LEN
		? `${query.slice(0, USER_QUERY_TRUNCATE_LEN)}...`
		: query;
}
...
async function buildPage(
	...
	const [conversations, totalCount] = await Promise.all([
		getConversationsByUserId(userId, perPage, offset),
 ...
 * scanning-and-deleting matching keys.
 *
...
export default async function handleRequest(
	...
	const cacheKey = `harness-conversations:list:${userId}:${version}:${page}:${perPage}:${needUserQuery}`;
	...
	if (cached) {
		...
		base = await buildPage(userId, page, perPage, needUserQuery);
