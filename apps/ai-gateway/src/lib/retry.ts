...
export function rateLimitDelayMs(error: unknown, attempt = 1): number | null {
	...
	if (Number.isFinite(seconds) && seconds > 0) return Math.min(seconds * 1000, RATE_LIMIT_MAX_MS);
	return Math.min(RATE_LIMIT_BASE_MS * 2 ** (Math.max(attempt, 1) - 1), RATE_LIMIT_MAX_MS);
}
...
export async function withRetry<T>(
	...
	const { maxRetries = 3, baseDelayMs = 500, maxDelayMs = 10000, factor = 2 } = options;
	...
	while (true) {
		try {
			...
			const delay =
				...
				Math.min(baseDelayMs * factor ** (attempt - 1), maxDelayMs);
			const errorMessage = error instanceof Error ? error.message : String(error);
			logger.warn(
				`[Retry] Operation failed (attempt ${attempt}/${maxRetries}). Retrying in ${delay}ms...`,
				{ error: errorMessage },
			);
