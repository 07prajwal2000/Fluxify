...
Tokens use **markdown directive** syntax (`remark-directive`), so
`react-markdown` parses them natively — no custom regex parser on the frontend.
Every token is a **text directive**: `:name{attr="value" attr2="value2"}`.

Format rules for ALL tokens:
- Leading colon, token name, attributes inside `{ }`. No space between the name
  and `{`, no padding just inside the braces.
...
- The legacy `@name(...)` parenthesis form is **dead** — it does not render.
...
Note: previously positional (`@resource(type, identifier)`); both parts are now
named attributes.
