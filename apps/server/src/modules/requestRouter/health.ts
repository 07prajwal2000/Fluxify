...
let depsReady = false;
// Flipped true on shutdown, before anything is torn down. A draining node is
// still finishing the work it has; it just must not be given any more, so
// readiness goes 503 and the load balancer takes it out of rotation.
let draining = false;
...
/** One way only — a node that started draining is on its way out. */
export function markDraining() {
	draining = true;
}

export function isReady() {
	return depsReady && !draining && natsConnected();
}
 ...
 *              AND the node is not draining; 503 otherwise so traffic isn't
 *              routed to a half-loaded or departing worker.
 */
export function registerHealthRoutes(app: Hono<any>) {
	app.get("/_/admin/api/healthchecks/startup", (c) => c.body(null, 200));

	app.get("/_/admin/api/healthchecks/ready", (c) => c.body(null, isReady() ? 200 : 503));
}
