...
describe("buildDeployment", () => {
	...
	const deployment = (over: Partial<ClaimWorkload> = {}) =>
		buildDeployment(workload(over), options, shared, true);
	...
	it("stops the old pod before its replacement under a node cap, so a rollout at the cap finishes", () => {
		expect(spec(deployment()).strategy).toEqual({
			type: "RollingUpdate",
			rollingUpdate: { maxSurge: 0, maxUnavailable: 1 },
		});
	});

	it("keeps Kubernetes' surge-first rollout when the license has no node cap", () => {
		expect(spec(buildDeployment(workload(), options, shared, false))).not.toHaveProperty("strategy");
	});
	...
	it("restarts the pods when the shared settings change, since a Secret change alone does not", () => {
		const hash = (env: Record<string, string>) =>
			spec(buildDeployment(workload(), options, env, true)).template.metadata
...
describe("no string a user can write reaches the API server", () => {
	it("refuses a host that would end the Traefik rule and start another", () => {
		...
		expect(() => buildDeployment(bad, options, {}, true)).toThrow("malformed host");
	});
