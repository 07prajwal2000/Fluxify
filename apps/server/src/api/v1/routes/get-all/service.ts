import { and, eq, gt, gte, ilike, inArray, lt, lte, ne, type SQL, sql } from "drizzle-orm";
import type z from "zod";
import { type AuthACL, routesEntity } from "../../../../db/schema";
...
export default async function handleRequest(
	...
	const { result, totalCount } = await getRoutesList(offset, query.perPage, filterSQL);
...
function generateFilterSQL(query?: z.infer<typeof requestQuerySchema>): SQL {
	...
	if (filter?.projectId !== undefined) {
		...
		return and(eq(routesEntity.projectId, projectId), generateFilterSQL(query))!;
	}
	...
	if (filter.operator == "eq" && typeof value === "string") return ilike(field, filter.value);
